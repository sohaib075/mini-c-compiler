...
FLEX     = flex
BISON    = bison
CFLAGS   = -Wall -Wno-unused-function -g -Iinclude -Isrc
LDFLAGS  = 
...
# ── Source files ──
BISON_SRC = $(GRAM_DIR)/parser.y
FLEX_SRC  = $(GRAM_DIR)/lexer.l
...
# Build the runner utility
runner: runner.c
	$(CC) $(CFLAGS) -o runner runner.c

# Default: build everything
all: dirs $(TARGET) runner
	@echo "========================================"
	@echo "  Mini-C Compiler built successfully!"
	@echo "  Run: ./$(TARGET) [input.c]"
	@echo "  Or use interactive runner: ./runner"
	@echo "========================================"
...
$(BISON_C) $(BISON_H): $(BISON_SRC)
	$(BISON) -d -b $(SRC_DIR)/parser -o $(BISON_C) $(BISON_SRC)
	@if [ -f $(SRC_DIR)/parser.tab.h ]; then mv $(SRC_DIR)/parser.tab.h $(BISON_H); fi
...
clean:
	@rm -f $(TARGET).exe runner.exe $(BISON_C) $(BISON_H) $(FLEX_C) $(OUT_DIR)/output.asm
	@echo "Cleaned."
